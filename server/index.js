...
app.post('/orders/entry', verifyJWT,(req, res) => {
  const rows = req.body.rows;
   db.query('INSERT INTO customers (customer, product, quantity, price, total_price) VALUES ?',
   [rows.map((row) => [row.customer, row.product, row.quantity, row.price, row.totalPrice])],
      (err) => {
        if(err){
          res.send({status: 'error', err: err, message: "error occured"});
        }else{
          res.send({status: 'success', message: 'successfully submited'})
        }
      });
})

app.get('/orders/details', verifyJWT,(req, res) => {
  db.query('SELECT * FROM customers', (err, result) => {
    if(err){
      res.send({message: "faild to load resource", err: err})
    }else{
